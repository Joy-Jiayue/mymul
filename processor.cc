...
bool processor_t::is_supported_isa(uint32_t feature) {
    return isa->extension_enabled(feature);
}

bool processor_t::is_supported_xlen(uint32_t xlen) {
    return isa->get_max_xlen() == xlen;
}

void processor_t::trace_opcode(uint32_t opcode, insn_t insn) {
    reg_t pc = state.pc;
