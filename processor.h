...
//     insn_bits_t bits = insn.bits();
//     switch (bits) {
//       case MATCH_C_MYMUL:
//           return &processor_t::custom_mymul;
...
//   require_extension('C');
//   WRITE_RD(sext_xlen(RVC_RS1(p, insn) * RVC_RS2(p, insn)));
//   trace_opcode(p, MATCH_C_MYMUL, insn);
//   return pc + 2;  
// }
...
public:
    ...
    void trace_opcode(uint32_t opcode, insn_t insn);
...
void processor_t::trace_opcode(uint32_t t, reg_t pc, insn_t insn, reg_t next_pc) {
    ...
    printf("rd: x%2d=0x%016lx, rs1: x%2d=0x%016lx, rs2: x%2d=0x%016lx\n", insn.rd(), rd, insn.rs1(), rs1, insn.rs2(), rs2);
