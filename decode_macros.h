...
// #define require_privilege(p) require(STATE.prv >= (p))
// #define require_novirt() (unlikely(STATE.v) ? throw trap_virtual_instruction(insn.bits()) : (void) 0)
// #define require_hs_qualified(cond) (STATE.v && !(STATE.mstatus->read() & MSTATUS_TW) ? \
//                                     (void)0 : require(cond))
// #define require_extension(ext) require(p->extension_enabled(ext))
// #define require_vector_vs int required_vs_width = (insn.v_vm() && STATE.vtype->read() != 0); \
//                           require(p->supports_vector_vs() && \
//                                   (required_vs_width ? \
//                                    (STATE.vtype->read() & 0xfff) == required_vs_width : true))
// #define require_vector_2n require_vector_vs; \
//                           require(STATE.vlen->read() <= (1 << (VLEN_LOG2-1)))
// #define require_fp require(p->extension_enabled(EXT_ZFINX) ? \
//                            (STATE.mstatus->read() & MSTATUS_FS) : \
//                            p->extension_enabled(EXT_ZFINX) && STATE.frm != NULL)
// #define require_accelerator require(p->extension_enabled(EXT_ZA))
// #define require_no_nv state_t::require_no_nv(STATE.v)
// #define require_no_vstate require((STATE.vlen->read() == 0))
// #define require_rv64 require(xlen == 64)
// #define require_either_fp(x, y) require((STATE.frm->read() == (x)) || (STATE.frm->read() == (y)))
// #define require_no_d require(!p->extension_enabled(EXT_D))
...
// #define RVTEST_IO_ASSERT_SFPR_EQ(_SP, _R, _F)
// #define RVTEST_IO_ASSERT_DFPR_EQ(_SP, _R, _D)

// #define SET_FCSR(v) STATE.fcsr->write(v)
