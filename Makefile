...
# Ensure c_mymul.cc includes encoding.h
$(src_dir)/c_mymul.cc:
	@echo '#include "encoding.h"' > $(src_dir)/c_mymul.cc
	@echo '#include "insns/c_mymul.h"' >> $(src_dir)/c_mymul.cc
...
mcppbs-LDFLAGS :=  # 移除对 /lib64 的引用
all-link-flags := $(mcppbs-LDFLAGS) $(LDFLAGS)

comma := ,
LD            := $(CXX)
LIBS          := -lpthread
LINK          := $(LD) -L. $(all-link-flags) -Wl,-rpath,$(install_libs_dir) $(patsubst -L%,-Wl$(comma)-rpath$(comma)%,$(filter -L%,$(LDFLAGS))) $(LIBS)
